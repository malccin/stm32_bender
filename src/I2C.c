...
I2C_Status I2Cx_Init(I2C_TypeDef* I2Cx, uint32_t Clock) {
	...
	freq = (uint16_t)(24000000/ 1000000);
	I2Cx->CR2 |= freq;
	...
	if (Clock <= 100000) {
		...
		spd = (uint16_t)(24000000 / (Clock << 1)); // Duty cycle 50%/50%
		...
		spd = (uint16_t)(24000000 / (Clock * 3)); // Duty cycle 66%/33% (Tlow/Thigh = 2)
...
I2C_Status I2Cx_Read_byte(I2C_TypeDef* I2Cx, uint8_t *buf,
		uint8_t SlaveAddress, uint8_t numbytes) {
		...
		if (I2Cx_WaitEvent(I2Cx,I2C_EVENT_EV5) == I2C_ERROR){

			I2Cx->CR1 &= ~I2C_CR1_PE;

			I2Cx_Init(I2C1,100000);
...
I2C_Status I2Cx_Read(I2C_TypeDef* I2Cx, uint8_t *buf, uint32_t nbytes,
	...
	if (nbytes == 1) {
		...
		// Wait for RxNE flag (receive buffer not empty) EV7
		if (I2Cx_WaitFlagSet(I2Cx,I2C_F_RXNE) == I2C_ERROR) return I2C_ERROR;
		/*if (I2Cx_WaitFlagSet(I2Cx,I2C_F_RXNE) == I2C_ERROR){

							I2Cx->CR1 &= ~I2C_CR1_PE;

							I2Cx_Init(I2C2,100000);
		...
		*buf = (uint8_t)I2Cx->DR;
		...
		while (nbytes-- != 3) {
			...
			if (I2Cx_WaitFlagSet(I2Cx,I2C_F_BTF) == I2C_ERROR) return I2C_ERROR;
			/*if (I2Cx_WaitFlagSet(I2Cx,I2C_F_BTF) == I2C_ERROR){
...
I2C_Status I2Cx_Write_2(I2C_TypeDef* I2Cx, const uint8_t* buf, uint32_t nbytes,
		uint8_t SlaveAddress, I2C_STOP_TypeDef stop) {
